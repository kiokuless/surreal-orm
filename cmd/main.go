...
func commonTypeGeneration(f *File) {
	f.Type().Id("arrayJSON").Struct(Id("Inner").Index().Id("objectJSON").Tag(map[string]string{"json": "Array"}))
	f.Type().Id("idJSON").Struct(Id("Inner").Id("innerIDJSON").Tag(map[string]string{"json": "id"}))
	f.Type().Id("innerIDJSON").Struct(Id("Inner").Id("innerInnerIDJSON").Tag(map[string]string{"json": "Thing"}))
	f.Type().Id("innerInnerIDJSON").Struct(Id("ID").Id("idStringJSON").Tag(map[string]string{"json": "id"}), Id("TB").String().Tag(map[string]string{"json": "tb"}))
	f.Type().Id("idStringJSON").Struct(Id("String").String().Tag(map[string]string{"json": "String"}))
}
...
func generateStructByFields(packageName, structName string, fields []*ast.Field) string {
	...
	for _, field := range fields {
		...
		case "IDThing":
			fieldDecls = append(fieldDecls, Id(fieldName).Id("idJSON").Tag(map[string]string{"json": jsonName}))
		}
	}
	...
	for _, field := range fields {
		...
		case "IDThing":
			block = append(block, Id("s").Dot(fieldName).Dot("ID").Op("=").Id("j").Dot(fieldName).Dot("Inner").Dot("Inner").Dot("ID").Dot("String"))
			block = append(block, Id("s").Dot(fieldName).Dot("TB").Op("=").Id("j").Dot(fieldName).Dot("Inner").Dot("Inner").Dot("TB"))
		}
	}
	...
	commonTypeGeneration(f)

	return fmt.Sprintf("%#v", f)
}

func doNothing(_ any) {}
